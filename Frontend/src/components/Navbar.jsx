...
export default function Navbar() {
  ...
  return (
    <div className="flex w-full justify-between items-center py-6 relative">
      ...
      <ul className="hidden lg:flex items-center justify-between w-[300px]">
      ...
      <div className="relative hidden lg:block">
        {token ? (
          ...
          >
            ...
            {/* Dropdown */}
            {showProfile && (
              <div className="absolute top-12 right-0 min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4 text-base text-gray-500 font-medium z-20">
          ...
          <NavLink to="/Createaccount">
            <button className="flex items-center justify-center w-[180px] h-[45px] border-2 rounded-3xl bg-[#6677FF] text-white">
              Create account
            </button>
          </NavLink>
        )}
      ...
      {/* Mobile hamburger icon (only on small screens) */}
      <div className="lg:hidden" onClick={() => setIsOpen((prev) => !prev)}>
        {isopen ? (
          <FaTimes className="text-3xl cursor-pointer mr-4" />
        ) : (
      ...
      {/* Mobile menu */}
      {isopen && (
        <ul className="flex flex-col items-center justify-around fixed right-0 top-20 bg-black/20 backdrop-blur w-full h-[250px] z-50 lg:hidden">
          <li><NavLink to="/" onClick={() => setIsOpen(false)}>Home</NavLink></li>
          <li><NavLink to="/Alldoctors" onClick={() => setIsOpen(false)}>All doctors</NavLink></li>
